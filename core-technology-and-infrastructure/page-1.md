...
For a market with outcomes indexed by *i*, the invariant is:
...
Where:
- **rᵢ** = reserve (token balance) of outcome *i*
...
For markets with **3+ outcomes**, the engine uses a Newton iteration solver (up to 50 iterations, 1e-12 tolerance) to maintain the invariant across multi-dimensional swaps.
